...
fn main() {
    ...
    let temp_surface = sdl2::surface::Surface::load_bmp(Path::new("assets/flame.bmp")).unwrap();
    let texture = texture_creator.create_texture_from_surface(&temp_surface).unwrap();
    let center = Point::new(320,240);
    let mut source_rect = Rect::new(0, 0, 256, 256);
    let mut dest_rect = Rect::new(0,0, 256, 256);
    ...
    while running {
        ...
        source_rect.set_x((256 * ((ticks / 100) % 16) ) as i32);
        canvas.clear();
